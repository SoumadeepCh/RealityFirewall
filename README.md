...
| 2 | Extract AMAF feature vectors from each | Creates real `X` matrix |
| 3 | Call `train_model(X, y)` with real labels | Replaces synthetic distributions |
| 4 | Test on Celeb-DF (cross-dataset) | Validates generalization |
| 5 | Adjust `PLATT_A/B` on validation set | Recalibrates probability outputs |
...
| 10 | 🔜 Planned | Real labeled training data (FaceForensics++, Celeb-DF) |
